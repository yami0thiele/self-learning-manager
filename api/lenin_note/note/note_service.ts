...
import { EditNoteRecordData, NoteData, NoteRecordData } from "./note_data";
...
export default {
  ...
  findNoteData: async (ctx: TxContext) => {
    ...
    const transferedNoteRecords: { [id: number]: Array<{ note_column_id: number, content: string | null }> } = {};
    noteRecords.forEach(noteRecord => {
      if (noteRecord.note_columns_records == null) return;
      if (transferedNoteRecords[noteRecord.note_records.id] == null) transferedNoteRecords[noteRecord.note_records.id] = [];
    ...
    // レイアウト
    const noteColumns = await note_repository.findNoteColumns(note.note_template_id, ctx);
  ...
  writeRecordData: async (ctx: TxContext) => {
    const noteRecord = note_command.newNoteRecord(ctx.request.body as NoteRecordData | EditNoteRecordData);
    const registeredNoteRecord = await note_repository.createNoteRecord(noteRecord, ctx);
    const registeredNoteColumnRecords = await note_repository.upsertNoteColumnRecords(noteRecord, registeredNoteRecord, ctx);
    return {
