...
import { NoteData, NoteRecord, NoteRecordData } from "./note_data";
import * as schema from "../../db/schema/lenin-note-schema";
import { asc, eq, sql } from "drizzle-orm";

export default {
  create: async (noteData: NoteData, ctx: TxContext) => {
    const [registered] = await ctx.tx.insert(schema.notes).values([noteData]).returning();
    return registered;
  },
  ...
  findNoteRecords: async (noteId: number, ctx: TxContext) => {
    const noteRecords = await ctx.tx
      ...
      .leftJoin(schema.noteColumnsRecords, eq(schema.noteRecords.id, schema.noteColumnsRecords.note_record_id))
  ...
  createNoteRecord: async (noteRecordData: NoteRecordData, ctx: TxContext) => {
    const [registered] = await ctx.tx.insert(schema.noteRecords).values([{
  ...
  upsertNoteColumnRecords: async (noteRecordData: NoteRecordData, noteRecord: NoteRecord, ctx: TxContext) => {
    // TODO: 無関係なidペアが渡されたときに上書きされる問題を解消する。
    const updatedOrRegistered = await ctx.tx
      .insert(schema.noteColumnsRecords).values(
        ...
        set: {
          content: sql.raw(`excluded.${schema.noteColumnsRecords.content.name}`),
          updated_at: new Date().toISOString()
        }
      }).returning()
